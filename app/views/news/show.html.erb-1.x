...
<%= watcher_tag(@news, User.current) if defined? watcher_tag %>
...
<% if @news.respond_to?(:attachments) %>
...
<% if (@news.respond_to?(:commentable?) && @news.commentable?) || (!@news.respond_to?(:commentable?) && authorize_for('comments', 'create')) %>
